...
export const SchemaEndPointCreate = Type.Object({
  ...
  timeout: Type.Optional(
    Type.Integer({ description: "请求超时时间(毫秒)", minimum: 1, maximum: 300000 }) // Max 5 minutes
  ),
});
...
export const SchemaProbeResultCreate = Type.Object({
  ...
  status: Type.Optional(Type.Integer({ description: "HTTP状态码", minimum: 100, maximum: 599 })),
  responseTime: Type.Optional(
    Type.Integer({ description: "响应时间(毫秒)", minimum: 0, maximum: 300000 })
  ),
...
export type SchemaProbeResultCreateType = Type.Static<typeof SchemaProbeResultCreate>;
...
export type SchemaProbeResultDetailType = Type.Static<typeof SchemaProbeResultDetail>;

// Helper functions to create VO (Value Objects)
export const createServiceDetailVo = (data: Service): SchemaServiceDetailType => {
  return {
    id: data.id,
...
export const createEndPointDetailVo = (data: EndPoint): SchemaEndPointDetailType => {
  return {
    id: data.id,
    createdAt: data.createdAt.toISOString(),
    updatedAt: data.updatedAt.toISOString(),
...
export const createProbeResultDetailVo = (data: ProbeResult): SchemaProbeResultDetailType => {
  return {
    id: data.id,
    createdAt: data.createdAt.toISOString(),
